...
{
	uint8_t *out = NULL;
	int mod = data_len % AES_BLOCK_SIZE;
	uint8_t pad = AES_BLOCK_SIZE - mod;
	uint32_t i = 0;
	
	*out_len = data_len + AES_BLOCK_SIZE - mod;
	out = (uint8_t *)malloc(*out_len * sizeof(uint8_t) + 1);
	if (NULL == out) 
	{
		*out_len = 0;
		return NULL;
	}
	memset(out, 0, (*out_len * sizeof(uint8_t) + 1));
									  ...
									  uint32_t *outLen)
{
	...
	{
		AesEncrypt( &context, (in + i*AES_BLOCK_SIZE), (out+i*AES_BLOCK_SIZE));
	}
									  ...
									  uint32_t *outLen)
{
	...
	for ( i = 0; i < blockCount; ++i) 
	{
		AesDecrypt(&context, (data+i*AES_BLOCK_SIZE), (in+i*AES_BLOCK_SIZE));
	}
